...
- **The API key never touches the browser.** A server endpoint (`/api/ask`)
  holds `ANTHROPIC_API_KEY` and talks to the model; the page only talks to
  your own server. The original approach of calling Anthropic directly from
  the browser would have exposed the key and been blocked by CORS.
