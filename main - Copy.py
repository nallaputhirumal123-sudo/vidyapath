...
def _fallback_lesson(subject: str, level: str) -> dict:
    return {
        ...
        "steps": [
            ...
            "The site owner adds ANTHROPIC_API_KEY in the server settings.",
...
async def _call_anthropic(question: str, subject: str, level: str) -> dict:
    """Ask the model for a blackboard-shaped lesson. Server-side only."""
...
def ask_config(user: User = Depends(current_user)):
    ...
    return {"enabled": ASK_ENABLED, "model": ANTHROPIC_MODEL if ASK_ENABLED else ""}
...
async def ask_vidya(body: AskIn, user: User = Depends(current_user),
    ...
    try:
        lesson = await _call_anthropic(question, subject, level)
    except Exception as e:
        print(f"Ask Vidya call failed: {type(e).__name__}: {e}")
        raise HTTPException(
            status_code=503,
            detail="Vidya could not reach the board just now. Try again.",
        )
