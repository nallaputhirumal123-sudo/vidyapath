...
class Assignment(Base):
    ...
    class_id = Column(Integer, ForeignKey("classes.id", ondelete="CASCADE"),
                      nullable=False, index=True)
    title = Column(String(240), nullable=False)
    kind = Column(String(12), default="task")     # "lesson" | "task"
    lesson_slug = Column(String(60), default="")  # for kind == "lesson"
    body = Column(Text, default="")               # for kind == "task"
    due_date = Column(String(20), default="")     # ISO date string, optional
    created_at = Column(DateTime(timezone=True), default=now)


class AssignmentDone(Base):
    """Marks a custom task complete. Lesson assignments are tracked via the
    existing Progress table instead."""
    ...
    user_id = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"),
                     nullable=False, index=True)
    done_at = Column(DateTime(timezone=True), default=now)
    __table_args__ = (UniqueConstraint("assignment_id", "user_id", name="uq_asg_user"),)
...
def _assignment_status(db, assignments, user_id):
    """Return {assignment_id: bool done} for one student."""
    out = {}
    lesson_slugs = [a.lesson_slug for a in assignments if a.kind == "lesson" and a.lesson_slug]
    done_lessons = set()
    if lesson_slugs:
        rows = db.query(Progress.lesson_slug).filter(
            Progress.user_id == user_id, Progress.completed == True,  # noqa: E712
            Progress.lesson_slug.in_(lesson_slugs)).all()
        done_lessons = {r[0] for r in rows}
    task_ids = [a.id for a in assignments if a.kind == "task"]
    done_tasks = set()
    if task_ids:
        rows = db.query(AssignmentDone.assignment_id).filter(
            AssignmentDone.user_id == user_id,
            AssignmentDone.assignment_id.in_(task_ids)).all()
        done_tasks = {r[0] for r in rows}
    for a in assignments:
        out[a.id] = (a.lesson_slug in done_lessons) if a.kind == "lesson" else (a.id in done_tasks)
    return out


def _asg_json(a):
    return {"id": a.id, "title": a.title, "kind": a.kind,
            "lesson_slug": a.lesson_slug, "body": a.body,
            "due_date": a.due_date}
...
def create_class(body: ClassIn, user: User = Depends(teacher_user),
    ...
    klass = Klass(name=body.name.strip()[:160], join_code=_gen_join_code(db),
                  ...
                  schedule=body.schedule.strip()[:4000])
    db.add(klass)
    db.commit()
    db.refresh(klass)
...
def my_classes(user: User = Depends(teacher_user), db: Session = Depends(get_db)):
    ...
    for k in classes:
        ...
        out.append({"id": k.id, "name": k.name, "join_code": k.join_code,
                    "students": n, "assignments": a, "schedule": k.schedule})
    return {"classes": out}
...
def class_detail(cid: int, user: User = Depends(teacher_user),
    ...
    done_counts = dict(db.query(Progress.user_id, func.count(Progress.id))
                       .filter(Progress.completed == True)  # noqa: E712
                       .group_by(Progress.user_id).all())
    roster = []
    for cm, u in members:
        st = _assignment_status(db, assignments, u.id)
        roster.append({
            "id": u.id, "name": u.name, "email": u.email,
            "lessons_done": done_counts.get(u.id, 0),
            "pct": round(done_counts.get(u.id, 0) / total_lessons * 100),
            "assignments_done": sum(1 for v in st.values() if v),
            "status": st,
        })
    roster.sort(key=lambda r: r["name"].lower())
    return {"id": k.id, "name": k.name, "join_code": k.join_code,
            "schedule": k.schedule,
            "assignments": [_asg_json(a) for a in assignments],
            "roster": roster}
...
def update_class(cid: int, body: ClassIn, user: User = Depends(teacher_user),
    ...
    k.schedule = body.schedule.strip()[:4000]
    db.commit()
    return {"ok": True}
...
def add_assignment(cid: int, body: AssignmentIn, user: User = Depends(teacher_user),
    ...
    if not k or (k.teacher_id != user.id and not user.is_admin):
        raise HTTPException(404, "Class not found")
    kind = "lesson" if body.kind == "lesson" else "task"
    a = Assignment(class_id=cid, title=body.title.strip()[:240], kind=kind,
                   lesson_slug=body.lesson_slug.strip()[:60] if kind == "lesson" else "",
                   body=body.body.strip()[:4000] if kind == "task" else "",
...
def my_enrolled_classes(user: User = Depends(current_user),
    ...
    for cm, k in memberships:
        ...
        st = _assignment_status(db, assignments, user.id)
        out.append({
            "id": k.id, "name": k.name, "school": k.school,
            "teacher": teacher.name if teacher else "",
            "schedule": k.schedule,
            "assignments": [{**_asg_json(a), "done": st.get(a.id, False)} for a in assignments],
        })
    return {"classes": out}


@app.post("/api/assignment/{aid}/done")
def toggle_task_done(aid: int, user: User = Depends(current_user),
                     db: Session = Depends(get_db)):
    a = db.get(Assignment, aid)
    if not a or a.kind != "task":
        raise HTTPException(400, "Only custom tasks are marked here")
    ...
    row = db.query(AssignmentDone).filter(
        AssignmentDone.assignment_id == aid, AssignmentDone.user_id == user.id).first()
    if row:
        db.delete(row)
        db.commit()
        return {"done": False}
    db.add(AssignmentDone(assignment_id=aid, user_id=user.id))
    db.commit()
    return {"done": True}
